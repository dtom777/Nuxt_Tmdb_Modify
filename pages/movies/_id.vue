  ...
  <v-container fluid>
    <v-row>
      <v-col align-self="center" cols="12" sm="8">
        ...
        <h3
          class="subtitle-1 font-weight-light font-italic"
          v-show="movie.tagline"
        >
          〜{{ movie.tagline }}〜
        </h3>
...
export default {
  ...
  computed: mapGetters(["getLoginUser"]),
  async asyncData({ $axios, $config, params }) {
    const movie = await $axios.$get(
      `${$config.apiUrl}/movie/${params.id}?api_key=${$config.tmdbApiKey}`
    );
    const credits = await $axios.$get(
      `${$config.apiUrl}/movie/${params.id}/credits?api_key=${$config.tmdbApiKey}`
    );
    return {
      movie,
      genres: movie.genres.map(obj => obj.name),
      credits: credits.cast.map(obj => obj.name)
    };
  },
  methods: {
    preserve(date) {
